...
layer: ⑥底座
status: 按需
purpose: Refore 系网页转设计 MCP(npm @refore-ai/html-to-mastergo-mcp,v0.3.1):agent 打开页面逐个导入 MasterGo 画布,7 个导入/读画布/导航工具,刻意只写不读改——文档修改交给同系 talk-to-design-mcp
...
Refore(挂 MasterGo 社区插件生态)的网页转设计 MCP:npm 包 @refore-ai/html-to-mastergo-mcp v0.3.1,配合 MasterGo 内的"Refore 网页转设计"插件把 HTML 文件或网页 DOM 导入 MasterGo 画布。设计意图是批量场景——让 agent 按要求走完一个操作流程/所有子模块,逐页导入形成完整设计稿。工具分三组:导入(import_html 首选入口,支持本地文件与浏览器 dump 两类来源、target 定位插入/替换、get_capture_guide 抓取 SOP、no_browser_fallback 兜底)、读画布自检(get_design_context/query_nodes/get_local_styles/export_node_image 等七件)、导航(select_nodes/scroll_into_view/set_current_page)。
